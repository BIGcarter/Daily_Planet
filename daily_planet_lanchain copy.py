...
from langchain_core.output_parsers import StrOutputParser # Changed from JsonOutputParser for summarization initially
# JsonOutputParser might still be useful if we want to use its built-in retry/fix mechanisms,
# but for now, we'll parse manually after getting the string.

# --- Configuration (matches original script) ---
SILICONFLOW_API_KEY = "sk-psdwxdblqsanpckjwnkoebqnvpgszjdahyvqpgbexltvrkpx" # 请替换为您的真实密钥或使用环境变量
...
TOPIC = "Star formation / Planet formation: star formation in the Milky Way, including the formation of protostellar/protoplanetary disks. Both theoretical and observational papers."
OUTPUT_DIR = './astro_ph_daily_picks_langchain_robust' # Changed output dir

# --- Pydantic Models for Structured Output (for Summarization) ---
...
# Classification Prompt (remains the same)
...
# Summarization Prompt (remains the same)
...
# Summarization Chain - now outputs a string first
...
def fix_all_latex_backslashes_in_dict(obj: Dict[str, Any]) -> Dict[str, Any]:
    """
    遍历字典所有值，凡是字符串中出现 LaTeX 反斜杠 `\` 的都替换为 `\\`
    """
        ...
        try:
            ...
            if parsed_json_list:
                ...
                for item_dict in parsed_json_list:
                    try:
                        item_dict = fix_all_latex_backslashes_in_dict(item_dict)
...
#     """
#     修复 LaTeX 公式中的反斜杠，避免被 Markdown 或 Python 字符串误解释。
#     仅对 $...$ 区段中的反斜杠进行转义。
#     """
#     def replacer(match):
#         content = match.group(1)
#         # 替换单反斜杠为双反斜杠，避免被当作转义符丢失
...
def build_markdown(summaries: List[Dict[str, Any]], date: dt.date) -> str:
    ...
    for item in summaries:
        ...
        # summary_zh = fix_latex_backslashes(summary_zh_raw)  # ✅ 修复 LaTeX 转义问题
